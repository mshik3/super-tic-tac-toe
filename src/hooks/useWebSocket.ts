...
export interface UseWebSocketReturn {
  ...
  connect: () => void;
  disconnect: () => void;
  send: (message: import("../types/messages").ClientMessage) => boolean;
...
export function useWebSocket(
  ...
  const send = (
    message: import("../types/messages").ClientMessage
  ): boolean => {
    if (!wsRef.current) {
