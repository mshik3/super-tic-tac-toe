import type { GameState, GameBoard, PlayerSymbol, SubBoard } from '../types/messages';
import { TOTAL_CELLS } from './winDetection';
import { checkSubBoardWinner, checkMainBoardWinner, isSubBoardCompleted, getValidMoves, isGameDraw } from './winDetection';
...
export function isValidMove(gameState: GameState, move: Move): boolean {
	...
	if (boardIndex < 0 || boardIndex >= TOTAL_CELLS || cellIndex < 0 || cellIndex >= TOTAL_CELLS) {
		return false;
	}
 ...
 */
function calculateNextActiveBoard(cellIndex: number, subBoards: SubBoard[]): number | null {
...
export function applyMove(gameState: GameState, move: Move): MoveResult {
	...
	const subBoardWinner = checkSubBoardWinner(newGameState.board.sub[boardIndex]);
	if (subBoardWinner) {
		newGameState.board.main[boardIndex] = subBoardWinner;
	} else if (newGameState.board.sub[boardIndex].every((cell) => cell !== null)) {
		// Sub-board is full but no winner (draw) - leave as null to indicate draw
	}

	// Calculate next active board
	newGameState.board.activeBoard = calculateNextActiveBoard(cellIndex, newGameState.board.sub);
 ...
 */
export function getValidMovesForGame(gameState: GameState): Array<{ boardIndex: number; cellIndex: number }> {
 ...
 * @returns Move object
 */
export function createMove(boardIndex: number, cellIndex: number, player: PlayerSymbol): Move {
	return {
		boardIndex,
		cellIndex,
		player,
